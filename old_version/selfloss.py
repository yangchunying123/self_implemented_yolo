...
class Loss(nn.Module):
    ...
    def forward(self, pred_tensor, target_tensor):
        ...
        no_obj_mask = (target_tensor[..., 4] == 0).unsqueeze(-1).expand_as(target_tensor) # actually, even one demension is label as "contains obj", but those predicted boxes that
                                                                                         # do not have largest iou with gt should also be regarded as no_obj. this should be dealed 
                                                                                         # with later
        gt_obj = target_tensor[obj_mask].view(-1, N).contiguous()     #(obj_num, N)
        pre_obj = pred_tensor[obj_mask].view(-1, N).contiguous()     #(obj_num, N)
        obj_num = gt_obj.size(0)  # num of objects contains in a image/batch

        gts =  gt_obj[ : , : 5].contiguous()     #(obj_num, 5)
        ...
        for  i in range(obj_num):
            ...
            for b in range(B):
                if b != index:
                    extra_conf.append(pre_obj[i, 5 * b + 4].item())
                else:
                    preds_posi[i] = pre_obj[i, 5 * b : 5 * b + 5]
        ...
        loss_xy = self.lambda_coord * F.mse_loss(gts[..., : 2], preds_posi[..., : 2], reduction='sum')
        loss_wh = self.lambda_coord * F.mse_loss(torch.sqrt(gts[..., 2: 4]), torch.sqrt(torch.clamp(preds_posi[..., 2: 4], 0, 1)), reduction='sum')
        loss_class = F.mse_loss(gt_obj[..., 5 * B : ], pre_obj[..., 5 * B :], reduction='sum')
        loss_conf_posi = F.mse_loss(gts[..., 4], preds_posi[..., 4], reduction='sum')
        loss_conf_nega = self.lambda_noobj * F.mse_loss(pre_nega_conf, torch.zeros_like(pre_nega_conf).cuda(), reduction='sum')
        # # Total loss
