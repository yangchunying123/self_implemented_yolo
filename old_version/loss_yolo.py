...
class Loss(nn.Module):
    ...
    def forward(self, pred_tensor, target_tensor):
        ...
        loss_noobj =  self.lambda_noobj * F.mse_loss(noobj_pred_conf, noobj_target_conf, reduction='sum')
        ...
        loss_xy = self.lambda_coord *  F.mse_loss(bbox_pred_response[:, :2], bbox_target_response[:, :2], reduction='sum')
        loss_wh = self.lambda_coord *  F.mse_loss(torch.sqrt(bbox_pred_response[:, 2:4]), torch.sqrt(bbox_target_response[:, 2:4]), reduction='sum')
