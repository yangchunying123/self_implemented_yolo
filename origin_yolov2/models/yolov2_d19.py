...
class YOLOv2D19(nn.Module):
    def __init__(self, device, input_size=416, num_classes=20, trainable=False, conf_thresh=0.001, nms_thresh=0.5, anchor_size=None):
        ...
        # backbone darknet-19
        self.backbone = build_backbone(pretrained=trainable)
    ...
    def inference(self, x):
        ...
        # reorg layer
        p5 = self.convsets_1(feats['layer3'])   #  ->[batch, 1024, H/32, W/32]
        p4 = self.reorg(self.route_layer(feats['layer2'])) #  ->[batch, 64, H/16, W/16] ->[batch, 256, H/32, W/32]
        p5 = torch.cat([p4, p5], dim=1) # ->[batch , 1024+256=1280, H/32, W/32]

        # head
        p5 = self.convsets_2(p5)# ->[batch , 1024, H/32, W/32]

        # pred
        pred = self.pred(p5)  # ->[batch , anchornum *(5+class), H/32, W/32]
    ...
    def postprocess(self, bboxes, scores):
        ...
        # NMS
        keep = np.zeros(len(bboxes), dtype=np.int8)
    ...
    def forward(self, x, target=None):
        if not self.trainable:
            ...
            # decode bbox
            x1y1x2y2_pred = (self.decode_boxes(reg_pred) / self.input_size).view(-1, 4)  #【batch , H * W* anchor , 4】-> [batch*H*W*anchor, 4]
